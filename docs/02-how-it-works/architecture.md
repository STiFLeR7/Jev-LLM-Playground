...
The fixed recorded-report endpoint reads only the curated historical report, replays it server-side, and returns an allowlisted projection. It accepts no client path. The independent browser explorer loads on demand and filters its local copy without changing or submitting the ticket form.
